function predictorPlot_in( T,pT,XXn,varargin )
% plot predictor
...
if ~isempty(find(Quad==4))
    p4=plot(x1(1:mid),XXn(Quad==4,indpred(1:mid))','b*');hold on    
    p=[p,p4(1)];str=[str,'pred T obs T'];
end
if ~isempty(find(Quad==1))
    p1=plot(x2(1:mid),XXn(Quad==1,indpred(1:mid))','ro');hold on
    p=[p,p1(1)];str=[str,'pred T obs F'];
end
if ~isempty(find(Quad==3))
    p3=plot(x2(1:mid),XXn(Quad==3,indpred(1:mid))','r*');hold on
...
if ~isempty(find(Quad==4))
    p4=plot(x1(mid:end),XXn(Quad==4,indpred(mid:end))','b*');hold on
    p=[p,p4(1)];str=[str,'pred T obs T'];
end
if ~isempty(find(Quad==1))
    p1=plot(x2(mid:end),XXn(Quad==1,indpred(mid:end))','ro');hold on
    p=[p,p1(1)];str=[str,'pred T obs F'];
end
if ~isempty(find(Quad==3))
    p3=plot(x2(mid:end),XXn(Quad==3,indpred(mid:end))','r*');hold on
    p=[p,p3(1)];str=[str,'pred F obs T'];
end
%legend([p2(1),p4(1),p1(1),p3(1)],'pred F obs F','pred T obs T','pred T obs F','pred F obs T')
set(gcf,'position',get(0,'screensize'))
