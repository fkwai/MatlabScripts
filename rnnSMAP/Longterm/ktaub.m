...
function [taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = ktaub(datain, alpha, wantplot,plotColor)
...
if wantplot ~= 0
    ...
    c=plotColor;
    lineW=1.5;
    %plot(datain(:,1),datain(:,2),c)
    hold on
    %updateTStick;
    %fixLables(gca,1);
    %xlabel('date yy');
    %ylabel('slope cm/y');
    plot(datain(:,1),slope,[c,'-'],'LineWidth',lineW)
